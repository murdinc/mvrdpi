package display

import (
	"image"
	_ "image/jpeg"
	_ "image/png"
	"math"
	"os"
	"time"
)

type Display struct {
	Dev    *SSD1306
	Height int
	Width  int
}

type Buf struct {
	Pix           []byte
	Width, Height int
}

var font []byte = []byte{
	,
}

func NewDisplay(iicBussAddr, devAddr byte, height, width int) *Display {
	lcdDev := NewDevice()
	lcdDev.Init(iicBussAddr, devAddr, height, width)
	lcdDev.InitDevice()

	return &Display{
		Dev:    lcdDev,
		Height: height,
		Width:  width,
	}
}

func imgToBitmap(fname *string) (buf *Buf, err error) {
	var f *os.File
	if f, err = os.Open(*fname); err != nil {
		return
	}

	var img image.Image
	img, _, err = image.Decode(f)
	if err != nil {
		return
	}
	rect := img.Bounds().Max

	h := (rect.Y + 7) / 8

	buf = &Buf{
		Width:  rect.X,
		Height: rect.Y,
		Pix:    make([]byte, rect.X*h),
	}

	for i := 0; i < h; i++ {
		for j := 0; j < rect.X; j++ {
			by := uint32(0)
			for k := 0; k < 8; k++ {
				c := img.At(j, i*8+k)
				r, g, b, _ := c.RGBA()
				y := (r + g + b) / 3
				if y < 65536/2 {
					by |= 1 << uint32(k)
				}
			}
			buf.Pix[i*buf.Width+j] = byte(by)
		}
	}

	return
}

func (d *Display) writeChar(char *byte, pos *int) {
	fontOffset := int(*char) * 5
	for i := 0; i <= 4; i++ {
		charSlice := font[fontOffset+i]
		d.Dev.WriteData(charSlice, *pos+i)
	}
}

func (d *Display) WriteText(text string, pos int) {
	for _, char := range []byte(text) {
		d.writeChar(&char, &pos)
		pos += 6
	}
	d.Dev.Display()
}

func (d *Display) WriteImageFile(fileName string, pos int) {
	img, err := imgToBitmap(&fileName)
	if err != nil {
		panic(err)
	}
	var currPos, currRow int
	for _, val := range img.Pix {
		if currPos%img.Width == 0 {
			currPos = 0
			currRow++
		}
		d.Dev.WriteData(val, pos+currPos+(d.Width*(currRow-1)))
		currPos++
	}
}

func (d *Display) MVRDLogo() {
	d.WriteImage(logo)
	time.Sleep(time.Second * 3)
	d.Clear()
}

func (d *Display) Clear() {
	d.Dev.Clear()
}

func (d *Display) WriteImage(img []byte) {
	d.Dev.Clear()

	var currPos, currRow int
	for _, val := range img {
		if currPos%d.Width == 0 {
			currPos = 0
			currRow++
		}
		d.Dev.WriteData(val, currPos+(d.Width*(currRow-1)))
		currPos++
	}

	d.Dev.Display()
}

func (d *Display) WriteHLine(pos, length int) {
	for i := 0; i < length; i += 1 {
		d.Dev.WriteData(0x10, i+pos)
	}
}

func (d *Display) WriteVLine(pos, rows int) {
	for i := 0; i < d.Width*rows; i += d.Width {
		d.Dev.WriteData(0xFF, pos+i)
	}
}

func (d *Display) WriteHDots(pos, length int) {
	for i := 0; i < length; i += 2 {
		d.Dev.WriteData(0x10, i+pos)
	}
}

func (d *Display) WriteVDots(pos, rows int) {
	for i := 0; i < d.Width*rows; i += d.Width {
		d.Dev.WriteData(0xAA, pos+i)
	}
}

func (d *Display) ClearArea(pos, length, rows int) {
	for row := 0; row < rows; row++ {
		for i := 0; i < length; i++ {
			d.Dev.WriteData(0x00, (row*d.Width)+pos+i)
		}
	}
}

func (d *Display) ClearImageArea(fileName string, pos int) {
	img, err := imgToBitmap(&fileName)
	if err != nil {
		panic(err)
	}
	rows := math.Ceil(float64(img.Height) / 8.0)
	d.ClearArea(pos, img.Height, int(rows))
}

var logo []byte = []byte{
	,
}
